...
func ParseIdentifier(s string) (Identifier, error) {
	...
	if len(parts) > 2 {
		return "", fmt.Errorf("invalid Identifier: too many colons in %q", s)
	}
	path := parts[len(parts)-1]
	if len(parts) == 2 && parts[0] != "minecraft" && parts[0] != "" {
		return "", fmt.Errorf("invalid Identifier: unsupported namespace %q", parts[0])
	}
	if path == "" {
		return "", fmt.Errorf("invalid Identifier: empty path in %q", s)
	}
	for _, c := range path {
		if !((c >= '0' && c <= '9') || (c >= 'a' && c <= 'z') || c == '_' || c == '-' || c == '.' || c == '/') {
			return "", fmt.Errorf("invalid Identifier: forbidden character %q in path %q", c, path)
		}
	}
...
func (i *Identifier) ReadFrom(r io.Reader) (n int64, err error) {
	...
	parsed, perr := ParseIdentifier(string(value))
	if perr != nil {
		return n, perr
	}
	*i = parsed
...
// WriteTo emits the identifier as `minecraft:path`. This codebase assumes the
// minecraft namespace; custom namespaces are not supported.
